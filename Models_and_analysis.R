...
####  Code to run models published in O'Neill, Bradley, and Allen 2021 Biological Invasions
##    Hotspots of invasive plant abundance are geographically distinct from hotspots of establishment
#     Created by M.W. O'Neill, last updated 14 December 2020 
 

#### files needed
## 1. environmental_layers folder
## 2. bias file 
## 3. Table with species names
## 4. occurence + abundance dataset for each species
## 5. MaxEnt must be installed on your machine

setwd("Folder") ## set the working directory to the downloaded folder of data 
...
## make table of species with their number of test and training points 
...
setwd("Folder") ## set working directory back to main data folder
...
for(s in 1:155){ ## Loop through species list
  ...
  ##initialize master dataframe of pairwise correlations with template
  ...
  ## remove formulas with large amounts of variables for the number of data points 
...
bioD <- as.data.frame(bio) ## make dataframe version of environmental variables
summary(bioD)
